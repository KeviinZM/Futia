...
import { formations } from '@/data/formations'; // IMPORT DES FORMATIONS
...
export default function SquadBuilder() {
  ...
  // On initialise l'équipe avec la formation par défaut, mais sans joueurs (player: null)
  // On ajoute un champ 'player' à chaque slot de la formation
  ...
  // --- SAUVEGARDE AUTOMATIQUE (Avec mémorisation de la formation) ---
  useEffect(() => {
    const savedData = localStorage.getItem('futia-squad-v2'); // Nouvelle clé v2 pour éviter conflits
    if (savedData) {
      try {
  ...
  const handleFormationChange = (e) => {
    ...
    // On essaie de garder les joueurs !
    // On prend les joueurs actuels et on essaie de les remettre dans les nouveaux slots
    // C'est une logique simple : on remplit les slots un par un
    const currentPlayers = squad.map(s => s.player).filter(p => p !== null);
    
    const newSquad = newLayout.map((slot, index) => {
      // S'il reste des joueurs à placer, on en met un, sinon null
  ...
  const handleResetSquad = () => {
    if (confirm("Effacer l'équipe ?")) {
      ...
      // On ne change pas la formation, juste les joueurs
    }
  };

  // --- LOGIQUE DE POSTES (Mise à jour pour inclure plus de postes) ---
  const positionMapping = {
    'G': ['G'],
    'DG': ['DG', 'DLG'], 'DD': ['DD', 'DLD'], 'DC': ['DC'],
    'DLG': ['DG', 'DLG', 'MG'], 'DLD': ['DD', 'DLD', 'MD'], // Ajout pour 5 défenseurs
    'MC': ['MC', 'MDC', 'MOC'], 'MDC': ['MDC', 'MC'], 'MOC': ['MOC', 'MC', 'AT'],
    'MG': ['MG', 'AG', 'AVG'], 'MD': ['MD', 'AD', 'AVD'],
    'AG': ['AG', 'AVG', 'MG'], 'AD': ['AD', 'AVD', 'MD'], 
    'AVG': ['AG', 'AVG', 'BU'], 'AVD': ['AD', 'AVD', 'BU'], // Pour 4-3-2-1
    ...
    <div className="relative w-full max-w-2xl mx-auto">
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-xl p-4 mb-6 shadow-xl text-white">
        ...
        <div className="flex justify-between items-center mb-4 border-b border-slate-700 pb-4">
            <label className="text-sm font-bold text-slate-400 mr-2">Formation :</label>
            <select 
                value={currentFormation} 
                onChange={handleFormationChange}
                className="bg-slate-800 border border-slate-600 text-white text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5"
            >
      ...
      <div className="h-[600px] bg-green-800 rounded-xl border-4 border-white/20 shadow-2xl overflow-hidden relative transition-all duration-500">
        ...
        {squad.map((slot) => {
            ...
            className="absolute transform -translate-x-1/2 -translate-y-1/2 flex flex-col items-center cursor-pointer group hover:scale-110 transition-all duration-500 ease-in-out z-10"
            ...
            {slot.player && (
                <div className="absolute -bottom-3 z-20 flex gap-0.5 bg-black/80 px-1 py-0.5 rounded shadow-sm border border-white/10">
                    {[1, 2, 3].map(i => (<div key={i} className={`w-2 h-2 transform rotate-45 border border-black ${i <= chem ? 'bg-blue-400' : 'bg-slate-600'}`}></div>))}
                </div>
            )}
        ...
        <div className="absolute inset-0 z-50 bg-slate-900/95 backdrop-blur-sm rounded-xl p-4 flex flex-col">
            ...
            <div className="overflow-y-auto flex-1 space-y-2 pr-2 scrollbar-thin scrollbar-thumb-slate-700">
                {availablePlayers.length > 0 ? (
                    ...
                    <div className="text-center text-slate-500 py-10"><p>Aucun joueur trouvé.</p></div>
                )}
            </div>
        </div>
      )}
