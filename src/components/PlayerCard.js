...
// NOTE : On utilise bien "export default" ici
  ...
  return (
    <Link href={`/player/${player.id}`} className="block">
      <div className="relative group w-64 h-80 bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl border-2 border-slate-700 shadow-xl overflow-hidden hover:scale-105 transition-all duration-300 hover:shadow-purple-500/50 hover:border-purple-500 cursor-pointer">
        ...
        {/* Infos Haut Gauche (Note & Position) */}
        <div className="absolute top-4 left-4 z-10 text-white">
          ...
          <div className="mt-2 space-y-1">
             {/* Drapeaux et Clubs */}
             <div className="w-6 h-6 bg-white rounded-full overflow-hidden p-0.5">
               <img src={player.nation_img} alt="Nation" className="w-full h-full object-cover"/>
             </div>
             <div className="w-6 h-6 bg-white rounded-full overflow-hidden p-0.5">
               <img src={player.club_img} alt="Club" className="w-full h-full object-cover"/>
             </div>
          </div>
        </div>
          ...
          <div className="mt-2 flex justify-between items-center bg-purple-900/50 rounded px-2 py-1 border border-purple-500/30">
              ...
              <span className="text-sm font-bold text-green-400">
                {player.detailedStats ? (
                   /* Note : Ici on affiche juste une valeur si l'algo n'est pas importé */
                   player.meta_score || "..."
                ) : (
                   player.meta_score || "N/A"
                )}
              </span>
          </div>
        </div>
