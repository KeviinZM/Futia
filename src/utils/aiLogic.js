...
export function calculateMetaScore(player) {
    ...
    switch (player.position) {
      case "BU": case "AT": case "AG": case "AD":
  ...
  function calculateBasicScore(player) {
    ...
    if (!s) return "8.0";
    let score = 0;
    if (["BU", "AG", "AD", "AT"].includes(player.position)) score = (s.pac*0.4) + (s.sho*0.4) + (s.dri*0.2);
    else if (["DC", "DG", "DD", "DLG", "DLD"].includes(player.position)) score = (s.def*0.4) + (s.phy*0.4) + (s.pac*0.2);
